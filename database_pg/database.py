...
class Database:
    ...
    async def setup_tables(self) -> None:
        ...
        async def fill_all_tables_by_default(connection: PoolConnectionProxy):
            ...
            sql_query = f''' 
                INSERT INTO groups (name, linal_deadlines, angem_deadlines)
                VALUES
                    ('None', $1, $2);
            '''
            ...
            await connection.execute(sql_query, linal_deadlines, angem_deadlines)
            ...
            sql_query = f'''
                ...
                VALUES 
                    ...
                    ('Петров Иван Николаевич', 'petrov_ivan', '$2b$12$qh9/Z.fgrxc/WS7OSs.b5.vRgHxJa3C2KE1ZvxC9QV10Umjk/KoXi', 2),
                    ('Сидорова Мария Михайловна', 'sidorova_maria', '$2b$12$qh9/Z.fgrxc/WS7OSs.b5.vRgHxJa3C2KE1ZvxC9QV10Umjk/KoXi', 2);
            '''
            await connection.execute(sql_query)
            ...
            sql_query = f'''
                ...
                VALUES
                    (2, 3, $1, $5, {linal_substr_vars_n}, {angem_substr_vars_n}),
                    (3, 2, $2, $6, {linal_substr_vars_n}, {angem_substr_vars_n}),
                    (4, 3, $3, $7, {linal_substr_vars_n}, {angem_substr_vars_n}),
                    (5, 4, $4, $8, {linal_substr_vars_n}, {angem_substr_vars_n});
            '''
            this_is_bad_name_angem = [[] for i in range(4)]
            this_is_bad_name_linal = [[] for i in range(4)]

            for marks_list in this_is_bad_name_linal:
                for i in range(LINAL_LABS_COUNT):
                    r = random.randint(1, 5)
                    if r <= 2:
                        marks_list.append({"result": True, "approve_date": (datetime.strptime(linal_deadlines[i], '%Y-%m-%d') + timedelta(days=random.randint(0, 3))).strftime('%Y-%m-%d')})
                    elif r <= 4:
                        marks_list.append({"result": True, "approve_date": (datetime.strptime(linal_deadlines[i], '%Y-%m-%d') - timedelta(days=random.randint(0, 3))).strftime('%Y-%m-%d')})
                    else:
                        marks_list.append({"result": False, "approve_date": None})

            for marks_list in this_is_bad_name_angem:
                for i in range(ANGEM_LABS_COUNT):
                    r = random.randint(1, 5)
                    if r <= 2:
                        marks_list.append({"result": True, "approve_date": (datetime.strptime(angem_deadlines[i], '%Y-%m-%d') + timedelta(days=random.randint(0, 3))).strftime('%Y-%m-%d')})
                    elif r <= 4:
                        marks_list.append({"result": True, "approve_date": (datetime.strptime(angem_deadlines[i], '%Y-%m-%d') - timedelta(days=random.randint(0, 3))).strftime('%Y-%m-%d')})
                    else:
                        marks_list.append({"result": False, "approve_date": None})

            await connection.execute(sql_query, *[json.dumps(marks_list) for marks_list in this_is_bad_name_linal], *[json.dumps(marks_list) for marks_list in this_is_bad_name_angem])
    ...
    async def add_group(self, group: StudentGroup) -> None:
        async with self._pool.acquire() as conn:
            ...
            linal_deadlines = [(datetime.strptime('2025-03-14', '%Y-%m-%d') + timedelta(days=5*i)).strftime('%Y-%m-%d') for i in range(LINAL_LABS_COUNT)]
            angem_deadlines = [(datetime.strptime('2025-03-14', '%Y-%m-%d') + timedelta(days=5*i)).strftime('%Y-%m-%d') for i in range(ANGEM_LABS_COUNT)]
            group_row = await conn.fetchrow(sql_query, group.name, group.teacher_id, linal_deadlines, angem_deadlines)
    ...
    async def get_all_groups_and_students(self) -> list[(str, list[StudentWithResults])]:
        ...
        async with self._pool.acquire() as conn:
            sql_query = '''
                SELECT groups.name as group_name, groups.linal_deadlines as linal_deadlines, groups.angem_deadlines as angem_deadlines, users.name as student_name, students.linal_marks as linal_marks, students.angem_marks as angem_marks
                FROM students
                JOIN users ON students.user_id = users.id
            ...
            for row in rows:
                ...
                linal_deadlines = row['linal_deadlines']
                angem_deadlines = row['angem_deadlines']
                ...
                groups_and_students[group_name].append(
                    StudentWithResults(
                        ...
                        linal_deadlines=linal_deadlines,
                        angem_deadlines=angem_deadlines
                    )
                )
